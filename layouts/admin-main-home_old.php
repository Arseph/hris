                  ...
                  echarts.init(document.querySelector("#verticalBarChart")).setOption({
                    ...
                    series: [
                      <?php
                        ...
                        //get highest year
                        $get_high = "select TOP 1 * from HR_INFO order by date_hired desc";
                        $high_stmt = sqlsrv_query($conn, $get_high);
                        $high_row = sqlsrv_fetch_array($high_stmt);
                        $date_hired = $high_row['date_hired'];
                        $high_year = substr_replace($date_hired ,"",-6);
                        


                        //get highest year
                        $get_low = " select top 1 * from HR_INFO order by date_hired asc;";
                        $low_stmt = sqlsrv_query($conn, $get_low);
                        $low_row = sqlsrv_fetch_array($low_stmt);
                        $date_hired2 = $low_row['date_hired'];
                        $low_year = substr_replace($date_hired2 ,"",-6);

                        echo "high year: ".$high_year."<br>";
                        echo "low year: ".$low_year;


                        // for($l=$low_year; $l<=$high_year; $l++)
                        for($l=2018; $l<=$high_year; $l++)
                        {
                          $m = substr($date_hired2, 5,-3);

                          //count hired per month of every year
                          for($m=1; $m<=12; $m++)
                          {

                            $get_monthly = "select * from HR_INFO where date_hired='$l'ck = '$m'";
