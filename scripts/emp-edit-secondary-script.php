...
if(isset($_POST['btn_save']))
{
	...
	$options = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
		...
		$sql1 = "select top 1 * from emp_education_secondary where agencyid='$uid' and void=1 and graduate=1 and id!='$id'";
	...
	if($count_result<1)
	{
		...
		$radio_grad=$_POST['radio_grad'];
		 ...
		 $sql = "update emp_education_secondary set school=?, from_year=?, to_year=?, scholarship=?, graduate=? where id='$id'";
		 $params= array($txt_school,$txt_from,$txt_to,$txt_scholarship,$radio_grad, $id);

		 sqlsrv_query($conn,$sql,$params);

		  include "scripts/audit_emp_update_secondary.php";
