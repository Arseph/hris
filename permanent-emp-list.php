    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Permanent Employee list</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
...
<?php
                    ...
                    $active_stmt = sqlsrv_query($conn, $active_sql, $params, $options);
                    $count = sqlsrv_num_rows($active_stmt);
                    ...
                    if($count>0)
                    {
                      ...
                      {
                        $agencyid = $active_row['agencyid'];

                        $findperm_sql = "select * from emp_designation where agencyid='$agencyid' and void='1' and exit_date='To Present' and status='1'";

                        $findperm_stmt = sqlsrv_query($conn, $findperm_sql, $params, $options);
                        $countperm = sqlsrv_num_rows($findperm_stmt);

                       
                       
                        if ($countperm>0)
                        {
                          $perm_row = sqlsrv_fetch_array($findperm_stmt);

                           $perm_uid = $perm_row['agencyid'];


                           //get basic info
                           $mname = "";

                           $getbasic_sql = "select * from emp_basic where agencyid='$agencyid'";
                           $getbasic_stmt = sqlsrv_query($conn,$getbasic_sql);
                           $basic_row = sqlsrv_fetch_array($getbasic_stmt);
                           $sname = $basic_row['surname'];
                           $fname = $basic_row['firstname'];
                           if(!empty($basic_row['middlename'])){ $mname = $basic_row['middlename']; }
                           ...
                           echo '<td><div class="dropdown">';
                            ...
                            echo "<a href='update-basic.php?uid=".$basic_row['agencyid']."'>Basic Information</a>";

                            if($count_address!=0){
                               echo "<a href='update-address.php?uid=".$agencyid."'>Address</a>";
                            $completion_count++;
                            }

                            if($count_identification!=0){
                               echo "<a href='update-identification.php?uid=".$agencyid."'>Identification</a>";
                            $completion_count++;
                            }
                            ...
                            if($count_family!=0){
                               echo "<a href='update-family.php?uid=".$agencyid."'>Family</a>";
                            $completion_count++;
                            }

                            if($count_misc!=0){
                               echo "<a href='update-misc.php?uid=".$agencyid."'>Misc Info</a>";
                            $completion_count++;
                            }
                            ...
                            {
                              echo "<a href='emp-designation-history.php?uid=".$agencyid."'>Employee Designation</a>";
                            $completion_count++;
                            }

                            
                               echo "<a href='emp-education-history.php?uid=".$agencyid."'>Education</a>";
                      ...
                      //     $agencyid= $getbasic_row['agencyid'];
                      ...
                      //     //get identification
                      //     $identification_sql = "select * from emp_identification where agencyid='$agencyid'";
                      ...
                      //     //get designation
                      //     $checkdesignation_sql = "select * from emp_designation where agencyid='$agencyid'";
                      //     $designation_result = sqlsrv_query( $conn, $checkdesignation_sql , $paramm, $options);
                      //     $count_designation = sqlsrv_num_rows($designation_result);
                      //     $row_designation = sqlsrv_fetch_array($designation_result);
